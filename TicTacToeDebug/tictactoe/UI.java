...
public class UI
{
    ...
    public boolean isLegalMove(State state, int row, int col) { // state class must be state class not integer class
        return 1 <= row && row <= Constants.BOARD_SIZE &&
        1 <= col && col <= Constants.BOARD_SIZE &&
        state.getBoardCell(row, col) == Constants.BLANK; // now this is a methode for the state class 
    }
    ...
    public int getMoveRow(int whoseMove, String xName, String oName) {
        int row = 0; // ^ added two strings for xName and oName whoseMove remains integer ex -1
        while (row <= 0 || row >= 4) { // ^bluj no entialize variable note row
            try {
    ...
    public int getMoveCol(int whoseMove, String xName, String oName) { 
        int col = 0; // give value strt initialize 0?
        while (col <= 0 || col >= 4) {
            try {
    ...
    public void printMove(State state, int row, int col, String xName, String oName) { // Parenthesis fixed, too many end
        System.out.printf(
            Constants.PRINT_MOVE, 
            getXOrO(state.getWhoseMove()), 
            getPlayerName(state.getWhoseMove(), xName, oName), 
                row, 
                col
            );
    } 
    ...
    public void printWinner(State state) {
        System.out.printf(
            Constants.WINNER, 
            getXOrO(state.getWhoseMove()), // GetPlayerName is a UI function not a state function from UI not state class
            getPlayerName(state.getWhoseMove(), state.getXName(), state.getXName())
        );
    }
