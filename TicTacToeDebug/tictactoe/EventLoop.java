...
public class EventLoop extends UI {

  // Instance variables for the UI and State classes
  State state = new State();
  UI ui = new UI();
  int row, col;
  String xName; // variables xName and oName missing, needed for printMove
  String oName;
  
  public static void main(String[] args) {
      EventLoop eventLoop = new EventLoop();
      eventLoop.run(); // refering to run, previosly EventLoop
  }


  public void run() { // <--- & ^ changed from EventLoop to run
    while (state.getGameState() != Constants.QUIT_PROGRAM) {
      ...
      if (gameState == Constants.STANDBY) {
        ...
        state.setOName(ui.promptForName(Constants.O)); // o not y
        ...
        if (ui.isLegalMove(state, row, col)) { // ^ refer to GET_O_MOVE corrections same same
        ...
        row = ui.getMoveRow(state.getWhoseMove(), state.getXName(), state.getOName());
        col = ui.getMoveCol(state.getWhoseMove(), state.getXName(), state.getOName());
        if (ui.isLegalMove(state, row, col)) { // reajust () and add state.getX & OName() methodes
          state.setGameState(Constants.MAKE_MOVE);
        }

      } else if (gameState == Constants.MAKE_MOVE) {
        ui.printMove(state, row, col, xName, oName); // add xName and oName match argument length
